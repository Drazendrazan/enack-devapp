...
UpdateSkill = function(skill, amount)
    ...
    elseif Config.Notifications and  tonumber(amount) < 0 then
        if Config.NotifyType == "3d" then
            Notification("~g~-" .. amount .. " SHITTY NOTI SYSTEM ~s~" .. skill)
        elseif Config.NotifyType == 'qb' then
            QBCore.Functions.Notify(amount .. " XP to " .. skill, "error", 3500)
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("SKILL LOST", "-" .. amount .. " XP to " .. skill, 3500, 'info')
        end
    end
